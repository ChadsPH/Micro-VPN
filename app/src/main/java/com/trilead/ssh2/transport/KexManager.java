...
public class KexManager implements MessageHandler
{
	...
	public static String[] getDefaultKexAlgorithmList() {
		return new String[] { 
			Curve25519Exchange.NAME,
			Curve25519Exchange.ALT_NAME,
			"ecdh-sha2-nistp521",
			"ecdh-sha2-nistp384",
			"ecdh-sha2-nistp256",
	...
	public synchronized void handleMessage(byte[] msg, int msglen) throws IOException
	{
		...
		if (msg[0] == Packets.SSH_MSG_NEWKEYS)
		{
			...
			SkStatus.logInfo((new StringBuffer().append( "Key exchange algorithm: ").append(sci.keyExchangeAlgorithm +"\n").toString()));
		    SkStatus.logInfo(new StringBuffer().append("Using Algorithm: ").append(sci.serverHostKeyAlgorithm).toString());
			...
			|| kxs.np.kex_algo.equals("diffie-hellman-group-exchange-sha256"))
		{
			...
			if (kxs.state == 2)
			{
				...
				if (verifier != null)
				{
					...
					try
					{
						vres = verifier.verifyServerHostKey(hostname, port, kxs.np.kex_algo,  kxs.np.server_host_key_algo, kxs.hostkey);
					}
					catch (Exception e)
					{
